...
public class Parser {
    ...
    private Stmt ifStatement() {
        List<Stmt.If> ifChain = new ArrayList<>();
        ...
        if (!check(ELSE)) {
            ifChain.add(new Stmt.If(condition, thenBranch, null));
        }
        ...
        while (match(ELSE)) {
            if (match(IF)) {
                ...
                ifChain.add(new Stmt.If(condition, thenBranch, elseBranch));
                break;
            }
        }
        return new Stmt.IfChain(ifChain);
